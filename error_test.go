...
func TestError_NewServerError(t *testing.T) {
	setup()
	defer teardown()
	...
	err := NewServerError(resp, errors.New("Original http error"))
...
func TestError_NoResponse(t *testing.T) {
	err := NewServerError(nil, errors.New("Original http error"))
